...
public class PraktijkDAO {
	...
	public static Praktijk getPraktijk(int id) {
		...
		Document document = domdocument.getDocument("praktijken.xml",
				"praktijken.xsd");
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					if (Integer.parseInt(_id) == id) {
						...
						String adres = child
								.getElementsByTagName("adres").item(0)
								.getTextContent();
						String postcode = child
								.getElementsByTagName("postcode").item(0)
								.getTextContent();
						String plaats = child
								.getElementsByTagName("plaats").item(0)
								.getTextContent();
						String iban = child
								.getElementsByTagName("iban").item(0)
								.getTextContent();
						String telNr = child
								.getElementsByTagName("telnr").item(0)
								.getTextContent();
						String email = child
								.getElementsByTagName("email").item(0)
								.getTextContent();
						String website = child
								.getElementsByTagName("website").item(0)
								.getTextContent();
						
						praktijk = new Praktijk(id, naam, adres, postcode, plaats, iban, telNr, email, website);
					}
				}
			}
		} else
	...
	public static void setPraktijk(Praktijk praktijk) {
		...
		if (document != null) {
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				if (node instanceof Element) {
					...
					if (Integer.parseInt(_id) == praktijk.getId()) {						
						Node naam = child.getElementsByTagName("naam")
			...
			domdocument.writeDocument("praktijken.xml", "praktijken.xsd",
					document);
		} else
			System.out.println("XML document is null");
			...
			for (int i = 0; i < list.getLength(); i++) {
				...
				if (node instanceof Element) {
					Element child = (Element) node;
						...
						praktijken.add(new Praktijk(id, naam, adres, postcode, plaats, iban, telNr, email, website));
					}
			}
		} else
			System.out.println("XML document is null");
